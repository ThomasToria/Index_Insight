% Safe additions and targeted replacements for the existing thesis.
% This file is intentionally additive: it preserves the detailed Methodology
% already present in the main manuscript.

% =============================================================================
% 1. ABSTRACT: replace only the existing abstract environment with this block.
% =============================================================================
\begin{abstract}
...
% =============================================================================
% 2. INTRODUCTION: add at the END of "Objectives of the thesis".
% =============================================================================
...
% =============================================================================
% 3. METHODOLOGY: KEEP the existing "Data collection" section. Replace ONLY
% the current corpus source list and its table in "Corpus construction" with
% the following paragraph and table.
% =============================================================================
The corpus construction process combined the internal SIAGE corpus with publicly
accessible institutional and associative resources. The unit of analysis is one
documentary record: one web page, PDF description, or action sheet stored as one
text file. For each external source, a dedicated collection script was used
because page structures, navigation mechanisms, and available metadata differed.
Navigation pages, duplicated pages, empty pages, and pages without a meaningful
connection to territorial adaptation to ageing were excluded whenever they could
be identified automatically or during inspection.
...
source rather than the full French landscape.

% =============================================================================
% 4. METHODOLOGY: KEEP the existing "Cleaning and preprocessing",
% "Organization of the document database", "Indexing and search", and
% "Technical architecture" sections. Add this subsection at the END of
% "Technical architecture", immediately before \chapter{Implementation}.
% =============================================================================
\subsection{Extraction configuration, traceability, and scope}

The final extraction architecture combines the existing rule-based pipeline with
constrained LLM-assisted extraction. The current VADA extraction script calls
the local model \texttt{qwen2.5:1.5b} through the Ollama chat API. Local
inference was selected to avoid external API dependence, retain control over the
workflow, and keep iterative experimentation affordable. This model identifier
must be harmonised throughout the manuscript if another model was ultimately
used to generate the delivered database.

For each document, the script supplies a structured French prompt and requests
only valid JSON. The current configuration uses temperature 0, a context window
of 8,192 tokens, a maximum prediction length of 700 tokens, and an input
truncation threshold of 2,500 characters. The expected schema contains
...
\texttt{thematique}, and \texttt{source\_site}. Empty strings or lists are
required when a value cannot be supported by the document. Controlled
vocabularies are imposed for target audience and territorial scale, while concise
and homogeneous labels are requested for themes and initiative types.

Raw model responses are retained to support inspection. This improves
traceability but does not remove the need for human validation. In particular,
the input-truncation threshold may omit information located late in a document,
and a populated field can still be ambiguous or weakly supported. At this stage,
the project provides a structured corpus and exploratory outputs; it does not
yet provide a fully deployed search engine or a decision-making system.

% =============================================================================
% 5. IMPLEMENTATION: replace the current short chapter entirely.
% =============================================================================
...
The core workflow does not rely on an external commercial LLM API. However, a
model update, changed source page, or modified prompt can alter outputs. Raw
input text, generated JSON, and archived raw LLM answers are therefore essential
...
% =============================================================================
% 6. EVALUATION: replace the current short chapter entirely.
% =============================================================================
\chapter{Evaluation}
...
% =============================================================================
% 7. DISCUSSION, CONCLUSION, AND APPENDICES: replace the short chapters.
% =============================================================================
\chapter{Discussion}
