...
"""
Script d'analyse de la catégorie "thematique".

Sorties :
- Graphiques PNG dans :
  Index_Insight/outputs_graphs/outputs_graphs_thematiques/

- Fichier Excel dans :
  Index_Insight/outputs_graphs/outputs_graphs_thematiques/stats_thematiques.xlsx
"""
...
OUTPUT_DIR = BASE_DIR / "outputs_graphs" / "outputs_graphs_thematiques"
OUTPUT_EXCEL = OUTPUT_DIR / "stats_thematiques.xlsx"
    ...
    for json_path in json_files:
        try:
            ...
            thematiques = extract_values(
                data.get("thematique_normalisee", data.get("thematique"))
            )
...
def count_brut(rows, only_autres=False):
    ...
    for row in rows:
        ...
        for thematique in row["thematiques"]:
            counter[thematique] += 1
...
def count_pondere(rows, only_autres=False):
    ...
    for row in rows:
        ...
        unique_thematiques = sorted(set(row["thematiques"]))

        if not unique_thematiques:
            continue

        poids = 1 / len(unique_thematiques)

        for thematique in unique_thematiques:
            counter[thematique] += poids
...
def save_comparison_chart(counter_all, counter_autres, output_path, top_n=TOP_N):
    top_thematiques = [
        thematique for thematique, _ in counter_all.most_common(top_n)
    ]

    data = []

    for thematique in top_thematiques:
        data.append({
            "thematique": thematique,
            "toutes_fiches": counter_all.get(thematique, 0),
            "hors_fiches_actions": counter_autres.get(thematique, 0)
        })
    ...
    plt.title("Répartition des thématiques : avec / sans fiches actions")
...
def counter_to_df(counter):
    return pd.DataFrame(
        [
            ...
            for thematique, value in counter.most_common()
        ]
    )
...
def export_excel(
    ...
    for row in rows:
        rows_for_excel.append({
            ...
            "thematiques": ", ".join(row["thematiques"]),
            "path": row["path"]
        })
...
def main():
    ...
    if save_bar_chart(
        brut_all,
        "Thématiques — occurrences brutes",
        OUTPUT_DIR / "thematiques_brut_toutes_fiches.png"
    ):
        generated_graphs += 1

    if save_bar_chart(
        pondere_all,
        "Thématiques — occurrences pondérées",
        OUTPUT_DIR / "thematiques_pondere_toutes_fiches.png"
    ):
        generated_graphs += 1

    if save_bar_chart(
        pondere_autres,
        "Thématiques — pondéré hors fiches actions",
        OUTPUT_DIR / "thematiques_pondere_hors_fiches_actions.png"
    ):
        generated_graphs += 1
    ...
    comparison_df, comparison_ok = save_comparison_chart(
        pondere_all,
        pondere_autres,
        OUTPUT_DIR / "comparatif_thematiques_avec_sans_fiches_actions.png"
    )
